...
当前公开稳定版是 `v0.4.1`。只有通过一次由未参与开发者完成、且不绑定宿主系统的独立盲测，项目才会创建 `v0.5.0-rc1` 标签；开发团队继续在同一批材料上调参，不算发布证据。
...
5 张既有合成图生成原图、下采样、旋转、裁切、遮挡和 JPEG 往返共 30 个确定性输入。30 个唯一 `qwen3.5-ocr` 真实输出统一重放后，路由 30/30、危险误接收 0、字段精确一致 233/270、证据定位 266/270；选入最终结果的调用使用 71,259 Token，连同一次被中止的 7 项本地 PNG 失败尝试，本目标共使用 87,950 Token。人工修改业务数据仍为 0。完整输入、输出、失败和修正见[压力运行审计](evidence/qwen-ocr-stress-v0.4-report.md)。

`v0.4.1` 同时修复了共享裁切规则对旋转 90 度坐标的判断。保留的一条真实响应原先在仅 5/9 字段精确一致时被接收；修复后无需再次调用模型，即转入 `HUMAN_REVIEW / RULE_ALIGNED_RIGHT_EDGES`。在干净克隆中运行 `node replay-qwen-stress.mjs`，即可对 30 条经过哈希绑定的紧凑输出重新执行门禁，无需再次调用模型。
...
| 层级 | 评测单位 | 结果 | 证据边界 |
|---|---|---:|---|
| 契约回归 | 结构化候选 | 20/20 | 解析、schema、定位和规则逻辑 |
...
| Qwen-OCR 开发压力运行 | 变换后的合成图片 | 路由 30/30；危险误接收 0；字段 233/270；定位 266/270 | 重复开发材料，不是隐藏集 |
